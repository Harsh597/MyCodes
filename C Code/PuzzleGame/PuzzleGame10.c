...
void wingame(int arr1[], int dimension, int moves, int prevmoves)
{
    int k = 0, key;
    int mid_col = (65) / 2 - 15;
    char buffer[5];
...
int ScoreData(int moves, int dimension)
{
    Score sc;
    FILE *fp;
    int found = 0;
    int prevmoves;
    fp = fopen("Score.txt", "ab+");
    if (fp == NULL)
    {
        perror(fp);
...
void update_box_value(int ismain, int moves, int arr1[], int dimension, int x1, int y1)
{
    ...
    if (win == 1 && ismain)
    {
        int prevmoves = ScoreData(moves, dimension);
        wingame(arr1, dimension, moves, prevmoves);
    }
}
